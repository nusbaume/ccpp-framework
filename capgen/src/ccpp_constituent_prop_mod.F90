module ccpp_constituent_prop_mod
  ...
  type, public, extends(ccpp_hashable_char_t) :: ccpp_constituent_properties_t
    ...
    real(kind=kind_phys), private :: min_val = 0.0_kind_phys
    ! molar_mass_val is the molar mass of the constituent (kg mol-1)
    real(kind=kind_phys), private :: molar_mass_val = kphys_unassigned
    ...
    ! framework_owns_me is set by the caller of ccpp_model_constituents_t%new_field
    !    (via set_framework_owned) when the caller has allocated the object on
    !    the heap and is transferring ownership to the framework. When .false.
    !    (default), the framework treats this object as caller-owned and will
    !    not deallocate it during reset/teardown.
    logical, private :: framework_owns_me = .false.
  contains
    ...
    ! Ownership flag for framework-side cleanup (see field comment above)
    procedure :: is_framework_owned => ccp_is_framework_owned
    procedure :: set_framework_owned => ccp_set_framework_owned
  ...
  type, public :: ccpp_model_constituents_t
    ...
    real(kind=kind_phys), allocatable :: vars_layer(:, :, :)
    real(kind=kind_phys), allocatable :: vars_layer_tend(:, :, :)
    real(kind=kind_phys), allocatable :: vars_minvalue(:)
...
contains
  ...
  subroutine ccp_instantiate(this, std_name, long_name, diag_name, units, &
    ...
    real(kind=kind_phys), optional, intent(in) :: default_value
    real(kind=kind_phys), optional, intent(in) :: min_value
  ...
  logical function ccp_is_framework_owned(this) result(owned)
    ! Return .true. if this object's storage was transferred to the framework
    !    (via set_framework_owned) and should be deallocated during teardown.

    class(ccpp_constituent_properties_t), intent(in) :: this

    owned = this%framework_owns_me

  end function ccp_is_framework_owned

  !#######################################################################

  subroutine ccp_set_framework_owned(this, value)
    ! Mark this object as owned by the framework (value=.true.) when the
    !    caller has allocated it on the heap and is transferring ownership.
    !    Call before passing to ccpp_model_constituents_t%new_field.

    class(ccpp_constituent_properties_t), intent(inout) :: this
    logical,                              intent(in)    :: value

    this%framework_owns_me = value

  end subroutine ccp_set_framework_owned
  ...
  subroutine ccp_set_min_val(this, min_value, errcode, errmsg)
    ...
    ! Dummy arguments
    class(ccpp_constituent_properties_t), intent(inout) :: this
    real(kind=kind_phys), intent(in) :: min_value
    integer, optional, intent(out) :: errcode
    character(len=*), optional, intent(out) :: errmsg
  ...
  subroutine ccp_set_molar_mass(this, molar_mass, errcode, errmsg)
    ...
    ! Dummy arguments
    class(ccpp_constituent_properties_t), intent(inout) :: this
    real(kind=kind_phys), intent(in) :: molar_mass
    integer, intent(out) :: errcode
    character(len=*), intent(out) :: errmsg
  ...
  subroutine ccpt_deallocate(this)
    ! Release the wrapper's reference to its constituent property object.
    ! If the framework owns the object (i.e., the caller of
    !    ccpp_model_constituents_t%new_field allocated it and flipped its
    !    framework_owns_me flag via set_framework_owned), free its internal
    !    storage and deallocate the object itself. Otherwise, the object is
    !    caller-owned and we only drop our pointer to it.

    ! Dummy argument
    class(ccpp_constituent_prop_ptr_t), intent(inout) :: this

    if (associated(this%prop)) then
      if (this%prop%is_framework_owned()) then
