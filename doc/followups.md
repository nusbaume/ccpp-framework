...
| FU-036 | `test_io_helpers::test_idempotent_no_mtime_bump` fails on filesystems with 1-second mtime granularity | framework | 2026-09-08 | open | `unit-tests/test_io_helpers.py:118-126` sets the file's mtime to `time.time() - 3600` (a sub-second float) and asserts `os.path.getmtime` returns it **exactly**.  The temp dir comes from `tempfile.mkdtemp()`, i.e. `$TMPDIR` — on Derecho that is GPFS scratch, which truncates `os.utime` to whole seconds, so the assert compares `…919.0 != …919.556`.  Nothing to do with `open_if_changed`, which is behaving correctly (it did not rewrite the file).  1 of 1571 unit tests; passes wherever `$TMPDIR` has sub-second resolution, which is why CI has never seen it.  Fix is in the test, not the code: floor the timestamp (`float(int(time.time()) - 3600)`) or assert `<=` against a pre-call mtime rather than exact equality. |
...
| — | Framework: water-tracer properties are host-model-specialized | §4.19 | open |
| — | Capgen: the register phase is now visible to the host (`ccpp_scheme_const_properties`) | §4.20 | added 2026-09-08 |
...
| — | Q1–Q9 open design questions (`default_value` class, `water_species`, `mixing_ratio_type`, post-relaxation disagreement, `%instantiate` class-B args, singleton, `_layer` suffix, constituent triplet, water-tracer host-extended type) | §7 | open |
...
| FU-037 | `ccpp_scheme_const_properties` duplicate collapsing is not exercised at run time | 2026-09-08 | **Obsoleted by a design change the same day, before it was ever committed.**  Raised against the first cut of the routine, which returned one aggregated list across all registered suites with repeated standard names collapsed — a branch no in-tree suite could exercise, since none registers a repeated name.  The routine was then changed to report **one suite per call, verbatim** (`constituents_overhaul.md` §4.20), which deletes the collapse branch entirely rather than leaving it untested: deduplicating the union across suites is now the host's decision.  Kept as a row because the underlying observation still holds for anything that reintroduces cross-suite merging — the corpus cannot test it without a two-suite fixture registering the same constituent from both. |
...
|---------|-----------------|-----|
| `derecho1` | 2026-09-08 | **First sweep of this machine.**  Local stores held nothing durable: the auto-memory directory for this project is empty (no `MEMORY.md`, no memory files), there was no session/harness task list, and no `RESUME.md`-style scratch notes exist in the sibling working directories (`SE_projects/{water_isotopes,SIMA,atmospheric_physics,NCAR_ccpp-framework}`).  The one non-empty auto-memory store on this machine belongs to an unrelated project (MYNN-SFC).  Nothing to fold in, nothing to correct, no contradictions with this file.  Raised on the same visit, from porting `origin/water_tracer_mods` commit `7d87ae4` onto `feature/capgen-v1`: `constituents_overhaul.md` §4.19 + Q9 (water-tracer properties `water_tracer` / `prescribed_ratio` / `bulk_water_ind`, and whether they belong on the core type at all), the §4.2 / §4.3 / §5 / §9 updates those imply, and FU-036 (a unit-test portability failure this machine's GPFS `$TMPDIR` exposes).  Machine-local facts stayed local: this clone's out-of-band e2e build recipe (the harness's shell cannot load Lmod modules, so `cmake` needs `PATH` pointed straight at the spack Intel/OpenMPI 5.0.9 wrappers) went to auto-memory, not here.  **Second batch, same day** (host query for register-phase constituents): re-checked the local stores before adding rows — still nothing durable beyond the one memory this session wrote, so no fold-in.  Added §4.20 (`ccpp_scheme_const_properties`).  FU-037 was raised and closed within the same visit: it flagged an untestable duplicate-collapsing branch in the routine's first cut, and the switch to per-suite reporting removed that branch rather than leaving it untested — the row is kept in §4 so the observation survives for anything that reintroduces cross-suite merging. |
