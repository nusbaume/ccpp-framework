...
contains
  ...
  subroutine test_host(retval, test_suites)
    ...
    call const_props(index_dyn2)%is_water_tracer(check, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,a,i0,/,a)') "ERROR: Error, ", errcode, " trying ", &
    ...
    call const_props(index_dyn2)%bulk_water_index(check_index, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,a,i0,/,a)') "ERROR: Error, ", errcode, " trying ", &
    ...
    call const_props(index_dyn2)%prescribed_ratio(check_value, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,a,i0,/,a)') "ERROR: Error, ", errcode, " trying ", &
    ...
    ! instantiate call works as expected:
    call const_props(index_dyn1)%is_water_tracer(check, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,i0,/,a)') "ERROR: Error, ", errcode, &
    ...
    call const_props(index_dyn1)%prescribed_ratio(check_value, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,i0,/,a)') "ERROR: Error, ", errcode, &
    ...
    call const_props(index_dyn1)%bulk_water_index(check_index, errcode, errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,i0,/,a)') "ERROR: Error, ", errcode, &
    ...
    call const_props(index_dyn1)%set_bulk_water_index(index_dyn2, errcode, &
        errmsg)
    if (errcode /= 0) then
      write(6, '(a,i0,a,a,i0,/,a)') "ERROR: Error, ", errcode, " trying ", &
    ...
    if (errcode == 0) then
      call const_props(index_dyn1)%bulk_water_index(check_index, errcode, &
          errmsg)
      if (errcode /= 0) then
        write(6, '(a,i0,a,i0,/,a)') "ERROR: Error, ", errcode, &
            " trying to get bulk_water_index prop for dyn_const1 index = ", &
    ...
    call const_props(index_dyn2)%set_bulk_water_index(index_dyn1, errcode, &
        errmsg)
    if (errcode == 0) then
    ...
    call test_water_tracer%instantiate(std_name='not_a_water_tracer', &
        ...
        prescribed_ratio=0.5_kind_phys, errcode=errcode, errmsg=errmsg)
    if (errcode == 0) then
